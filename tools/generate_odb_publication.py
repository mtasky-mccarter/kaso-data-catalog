#!/usr/bin/env python3
"""Obchodné prípady reference publication from canonical YAML and SQL only.

Both renderers consume the same ordered blocks. Business registries are complete;
large technical graph/API/entity registries have explicit canonical source indexes.
...
SQL_DIR = Path("sql/diagnostic/obchodne-pripady")
EVIDENCE = Path("evidence/manifests/odb")
# An intermediate name required by the existing publication orchestrator.
BASENAME = "obchodne-pripady-v1.2"
...
def model(root):
    ...
    if reasons:
        raise ValueError("HANDOFF BLOCKED: " + "; ".join(reasons))
    ...
    for row in registry:
        ...
        if path.parent != (root / SQL_DIR).resolve() or path.suffix != ".sql":
            raise ValueError(f"SQL cesta mimo obchodných prípadov: {path}")
        sql[row["sql_id"]] = path.read_text()
    if {r["sql_file"] for r in registry} != {p.relative_to(root).as_posix() for p in (root / SQL_DIR).glob("*.sql")}:
        raise ValueError("SQL register nepokrýva kanonické SQL obchodných prípadov")
    ...
    return {"docs": docs, "evidence": evidence, "sql": sql, "version": version,
            "subject": "obchodné prípady", "digest": digest, "inputs": inputs}
...
def blocks(data):
    ...
    paragraph("Úplné manifesty a kontrolné súčty sú v kanonickom registri dôkazov. Nasledujúca tabuľka je index, nie náhrada manifestov.")
    output.append(("table", ["Manifest", "SHA-256 / súbor"],
                   [[str(m.get("evidence_id", n)), value_text({k:v for k,v in m.items() if "sha" in k or "file" in k})]
                    for n,m in data["evidence"].items()], [100,167]))
    heading("19 Dependency, API a prístupové hranice")
    document("dependencies.yaml", "Source potvrdené a prístupové hranice", dense=True)
    paragraph("Veľké dependency, API a Oracle entity registre sú publikované ako inventár. Úplné jednotlivé záznamy vrátane raw source loci zostávajú v uvedených kanonických YAML súboroch; táto časť je súhrn.")
    for name in sorted(set(docs) - consumed):
        ...
        if name.startswith(("api-references", "dependency-", "oracle-entities")):
            ...
            paragraph(f"Canonical source: {DOMAIN.as_posix()}/{name}")
            properties({k:v for k,v in doc.items() if k != "records"})
            paragraph(f"Počet záznamov: {len(doc.get('records', []))}")
        else:
...
def build_docx(data, output):
    ...
    header.text = f"KASO Data Catalog  |  obchodné prípady {data['version']}"
    ...
    doc.add_paragraph("Obchodné prípady", "Title")
    doc.add_paragraph(data["docs"]["contract.yaml"]["title_sk"], "Subtitle")
    doc.add_paragraph("Schválený MC kontrakt obchodných prípadov pre bezpečnú read-only diagnostiku.")
...
def build_pdf(data, output):
    ...
    story = [Spacer(1, 8 * mm), p("KASO Data Catalog", "title"), p("Technical & Diagnostic Reference", "h2"),
             p("Obchodné prípady", "title"), p(c["title_sk"], "h2"), p("Schválený MC kontrakt obchodných prípadov pre bezpečnú read-only diagnostiku."), Spacer(1, 6 * mm)]
    ...
    for block in blocks(data):
        if block[0] == "heading":
            ...
            _, headers, rows, widths = block
            # Long evidence/source-locus lists must not create page-tall rows.
            # Split only at existing newlines, preserving every cell value.
            expanded = []
            for row in rows:
                lines = [str(v).split("\n") for v in row]
                for offset in range(0, max(map(len, lines)), 15):
                    expanded.append(["\n".join(v[offset:offset + 15]) for v in lines])
            cooked = [[p(v, "head") for v in headers]] + [[p(v, "cell") for v in row] for row in expanded]
            table = LongTable(cooked, colWidths=[w * mm * 263 / 267 for w in widths], repeatRows=1, splitInRow=0, hAlign="LEFT")
    ...
    def footer(canvas, doc):
        ...
        canvas.drawString(15 * mm, 9 * mm, REPOSITORY + " | main | obchodné prípady " + data["version"])
    ...
    pdf = SimpleDocTemplate(str(output), pagesize=landscape(A4), leftMargin=15 * mm, rightMargin=15 * mm,
                            ...
                            title="Obchodné prípady", author=REPOSITORY)
...
def generate(root, output):
    ...
    payload = {
        ...
        "generator": "tools/generate_odb_publication.py", "generator_version": "1.0",
        "generated_from": "kanonické YAML obchodných prípadov, read-only SQL a manifesty dôkazov obchodných prípadov",
