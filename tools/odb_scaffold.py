...
def read_scaffold(root=ROOT):
    ...
    errors = validate_bundle(documents, root=root)
    if errors:
        raise ValueError('\n'.join(errors))
...
def publication_blockers(root=ROOT):
    ...
    reasons.append('HANDOFF BLOCKED: production publication materialization is not implemented in the scaffold phase')
    return reasons
